-- For each client IP, count the sum of uploaded, downloaded and total transmitted bytes.
...
B = FOREACH A GENERATE ip_c, unique_bytes_c, unique_bytes_s, unique_bytes_c + unique_bytes_s as total_bytes;
C = GROUP B BY ip_c;
D = FOREACH C GENERATE (group, SUM(B.unique_bytes_c), SUM(B.unique_bytes_s), SUM(B.total_bytes));
STORE D INTO 'output/ex3';
