...
B = LOAD 'output/ex4/' using PigStorage() as (ip:chararray, bytes:long);
C = JOIN B by ip, A by ip_c;
D = GROUP C BY ip;
E = FOREACH D GENERATE (group, MAX(C.A::unique_bytes_c)/(double)MAX(C.B::bytes));
DESCRIBE E;
STORE E INTO 'output/ex5';
